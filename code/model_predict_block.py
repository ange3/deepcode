...
def _build_net_layers(num_timesteps, num_asts, hidden_size, learning_rate, grad_clip=10, dropout_p=0.0, num_lstm_layers=1):
    ...
    return l_in, l_out, l_out_slice, l_lstm, l_lstm_slice
...
def create_model(num_timesteps, num_asts, hidden_size, learning_rate, grad_clip=10, dropout_p=0.5, num_lstm_layers=1):
    ...
    l_in, l_out, l_out_slice, l_lstm, l_lstm_slice = _build_net_layers(num_timesteps, num_asts, hidden_size, learning_rate, grad_clip, dropout_p, num_lstm_layers)
    ...
    generate_hidden_representations = theano.function([l_in.input_var, l_mask.input_var], l_lstm_slice, allow_input_downcast=True)

    print("Compiling done!")
    
    return train_loss_acc, compute_loss_acc, probs, generate_hidden_representations
...
def train(train_data, val_data, train_loss_acc, compute_loss_acc, num_epochs=5, batchsize=32, record_per_iter=False):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_acc, train_corrected_acc, pred_train = _compute_loss_acc_pred(X_train, mask_train, truth_train, compute_loss_acc)        
...
def _compute_loss_acc_pred(X, mask, truth, compute_loss_acc):
    ...
    # corrected_acc = compute_corrected_acc_on_ast_rows(X, mask, truth, pred)
...
def check_accuracy(data, compute_loss_acc, row_to_ast_id_map, dataset_name):
    ...
    print("  {} accuracy:\t\t{:.2f} %".format(dataset_name, acc * 100))
